...
// Pretty much the same as printMap(), but it prints blank slots on places that aren't yet generated. Used in chunk generation.
void Debug::printGeneratedMap() {
	int xMin = std::numeric_limits<int>::max(), xMax = std::numeric_limits<int>::min(), yMin = std::numeric_limits<int>::max(), yMax = std::numeric_limits<int>::min();
	...
	for (int y = yMax; y >= yMin; y--) {
		for (int x = xMin; x <= xMax; x++) {
			if (PlayerState::activeLevel.roomExists(x, y) && PlayerState::activeLevel.getRoom(x, y).generated) {
				if (PlayerState::pos == Point(x, y))
					std::cout << "x";
				else
					std::cout << "0";
